...
class linesearchRSGLC(constrained_optimization_solver):
  def __init__(self, dtype=jnp.float64) -> None:
    self.lk = None
    self.first_check = False
    self.active_num = 0
  ...
  def __iter_per__(self):
    ...
    if Mk is None:
      ...
      Md = Mk.T@d
    lr =self.__step_size__(Md,d,projected_grad,Mk,lr,alpha,beta)
    
    self.__update__(lr*Md)
    return
    
  def __step_size__(self,direction,projected_dir,projected_grad,Mk,lr,alpha,beta):
